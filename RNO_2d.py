...
class RNO_2D(nn.Module):
    def __init__(self, in_dim, out_dim, modes1, modes2, width, pad_amount=None, pad_dim='1'):
        ...
        self.p = nn.Linear(in_dim + 2, self.width) # input channel_dim is in_dim + 1: u is in-dim, and grid is 2 dim

        self.layer1 = RNO_layer(in_dim, out_dim, modes1, modes2, width, return_sequences=True)
        self.layer2 = RNO_layer(in_dim, out_dim, modes1, modes2, width, return_sequences=True)
        self.layer3 = RNO_layer(in_dim, out_dim, modes1, modes2, width, return_sequences=False)
