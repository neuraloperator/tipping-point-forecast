...
if __name__ == '__main__':
    ...
    path = 'MNO_airfoil_div_corrected_normalization_fixed_re1000_multi_step' + str(multi_step_num_steps) + '_N' +str(ntrain) + '_ep' + str(multi_step_ep) + '_1m' + str(modes1) + '_2m' + str(modes2) + '_w' + str(width) + 'domain_trunc' + \
    ...
    # New FNO Initialization
    model = FNO(
        n_modes=(modes1, modes2),
        hidden_channels=width,
        in_channels=dim, # Raw dim, embedding handles grid
        out_channels=dim, 
        n_layers=num_layers, 
    ...
    for ep in range(multi_step_ep):
        ...
        for n in tqdm(multi_step_list):
            ...
            # No history for MNO, just take the last step
            x = x[:,-1]
