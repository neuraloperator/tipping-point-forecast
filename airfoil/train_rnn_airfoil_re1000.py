...
if __name__ == '__main__':
    ...
    path = 'GRU_airfoil_div_corrected_normalization_fixed_re1000_multi_step' + str(multi_step_num_steps) + '_N' +str(ntrain) + '_ep' + str(multi_step_ep) + \
            '_drop' + str(dropout).replace('.', '_') + '_hidden_units' + str(hidden_units) + '_layers' + str(num_layers) + '_domain_trunc' + \
        ...
        for n in tqdm(multi_step_list):
            ...
            if model is None:
                model = GRU_spatial_net(in_channels=in_channels, out_channels=out_channels,
                                        ...
                                        hidden_units=hidden_units, num_layers=num_layers, dropout=dropout).to(device)
